...
direct_specular_OK(SCOLOR scval, FVECT ldir, double omega, BSDFDAT *ndp)
{
	...
	if (RCO_BSDF_INSTRUMENT) {
		fprintf(stderr,
			"RCO_BSDF_CPU_INSTRUMENT direct_specular_OK "
			"vin=(%.6g,%.6g,%.6g) vsrc=(%.6g,%.6g,%.6g) "
			"omega=%.9g tomega=%.9g "
			"scval=(%.9g,%.9g,%.9g) diffY=%.9g nsamp=%d scnt=%d\n",
			ndp->vray[0], ndp->vray[1], ndp->vray[2],
			vsrc[0], vsrc[1], vsrc[2],
			omega, tomega,
			scval[0], scval[1], scval[2],
			diffY, nsamp, scnt);
	}
	return(1);
baderror:
...
)
{
	...
	if (!direct_specular_only &&
			ldot > 0 && sintens(np->rdiff) > FTINY) {
		/*
	...
	if (!direct_specular_OK(sctmp, ldir, omega, np)) {
		if (RCO_BSDF_INSTRUMENT)
			fprintf(stderr, "RCO_BSDF_CPU_INSTRUMENT dir_bsdf FAIL "
				"ldir=(%.6g,%.6g,%.6g) omega=%.9g\n",
				ldir[0], ldir[1], ldir[2], omega);
		return;
	}
	if (RCO_BSDF_INSTRUMENT)
		fprintf(stderr, "RCO_BSDF_CPU_INSTRUMENT dir_bsdf OK "
			"sctmp=(%.9g,%.9g,%.9g) ldir=(%.6g,%.6g,%.6g) "
			"omega=%.9g ldot=%.9g\n",
			sctmp[0], sctmp[1], sctmp[2],
			ldir[0], ldir[1], ldir[2], omega, ldot);
...
)
{
	...
	if (!direct_specular_only && sintens(np->rdiff) > FTINY) {
		/*
		 *  Compute diffuse reflected component
		 */
		copyscolor(sctmp, np->rdiff);
...
)
{
	...
	if (!direct_specular_only && sintens(np->tdiff) > FTINY) {
		/*
		 *  Compute diffuse transmission
		 */
		copyscolor(sctmp, np->tdiff);
...
static int
sample_sdcomp(BSDFDAT *ndp, SDComponent *dcp, int xmit, int comp_i)
{
	...
	for (n = 0; n < nstarget; n++) {
		...
		if (RCO_BSDF_INSTRUMENT)
			fprintf(stderr,
				"RCO_BSDF_CPU_INSTRUMENT sample_sdcomp_draw "
				"samplendx=%lu parent_rno=%lu comp=%d xmit=%d n=%d "
				"nstarget=%d xrand=%.9g "
				"vinc=(%.6g,%.6g,%.6g) vsmp=(%.6g,%.6g,%.6g) "
				"bsvY=%.9g"
#if defined(_WIN32) || defined(_WIN64)
				" rng_state=%llu"
#endif
				"\n",
				samplendx, (unsigned long)ndp->pr->rno,
				comp_i, xmit, n, nstarget, xrand,
				vinc[0], vinc[1], vinc[2],
				vsmp[0], vsmp[1], vsmp[2],
				bsv.cieY
#if defined(_WIN32) || defined(_WIN64)
				, randstate()
#endif
		...
		if (hasthru) {			/* check for view ray */
			...
			if (dx*dx + dy*dy <= ndp->sr_vpsa[0]*ndp->sr_vpsa[0]) {
				if (RCO_BSDF_INSTRUMENT)
					fprintf(stderr,
						"RCO_BSDF_CPU_INSTRUMENT "
						"sample_sdcomp_skip_thru comp=%d "
						"xmit=%d dx=%.9g dy=%.9g sr=%.9g\n",
						comp_i, xmit, dx, dy, ndp->sr_vpsa[0]);
		...
		if (SDmapDir(sr.rdir, ndp->fromloc, vsmp) != SDEnone) {
			if (RCO_BSDF_INSTRUMENT)
				fprintf(stderr,
					"RCO_BSDF_CPU_INSTRUMENT sample_sdcomp_map_fail "
					"comp=%d xmit=%d\n", comp_i, xmit);
			break;
		}
		...
		if (rayorigin(&sr, xmit ? TSPECULAR : RSPECULAR, ndp->pr, sr.rcoef) < 0) {
			if (RCO_BSDF_INSTRUMENT)
				fprintf(stderr,
					"RCO_BSDF_CPU_INSTRUMENT sample_sdcomp_origin_fail "
					"samplendx=%lu parent_rno=%lu child_rno=%lu "
					"comp=%d xmit=%d rweight=%.9g "
					"coef=(%.9g,%.9g,%.9g)\n",
					samplendx, (unsigned long)ndp->pr->rno,
					(unsigned long)sr.rno, comp_i, xmit,
					sr.rweight,
					sr.rcoef[0], sr.rcoef[1], sr.rcoef[2]);
			if (!n & (nstarget > 1)) {
		...
		if (RCO_BSDF_INSTRUMENT)
			fprintf(stderr,
				"RCO_BSDF_CPU_INSTRUMENT sample_sdcomp_ray "
				"samplendx=%lu parent_rno=%lu child_rno=%lu "
				"comp=%d xmit=%d "
				"rorg=(%.9g,%.9g,%.9g) "
				"rdir=(%.6g,%.6g,%.6g) "
				"rop=(%.9g,%.9g,%.9g) rot=%.9g "
				"coef=(%.9g,%.9g,%.9g) hit_mod=%d rsrc=%d "
				"hit_obj=%d hit_otype=%d hit_oname=%s "
				"crtype=%u rcol_before=(%.9g,%.9g,%.9g)\n",
				samplendx, (unsigned long)ndp->pr->rno,
				(unsigned long)sr.rno, comp_i, xmit,
				sr.rorg[0], sr.rorg[1], sr.rorg[2],
				sr.rdir[0], sr.rdir[1], sr.rdir[2],
				sr.rop[0], sr.rop[1], sr.rop[2], sr.rot,
				sr.rcoef[0], sr.rcoef[1], sr.rcoef[2],
				sr.ro != NULL ? sr.ro->omod : -1, sr.rsrc,
				sr.ro != NULL ? objndx(sr.ro) : -1,
				sr.ro != NULL ? sr.ro->otype : -1,
				sr.ro != NULL ? sr.ro->oname : "(null)",
				(unsigned int)sr.crtype,
				sr.rcol[0], sr.rcol[1], sr.rcol[2]);
		smultscolor(sr.rcol, sr.rcoef);
		saddscolor(ndp->pr->rcol, sr.rcol);
		if (RCO_BSDF_INSTRUMENT)
			fprintf(stderr,
				"RCO_BSDF_CPU_INSTRUMENT sample_sdcomp_add "
				"comp=%d xmit=%d contrib=(%.9g,%.9g,%.9g) "
				"parent=(%.9g,%.9g,%.9g)\n",
				comp_i, xmit, sr.rcol[0], sr.rcol[1], sr.rcol[2],
				ndp->pr->rcol[0], ndp->pr->rcol[1], ndp->pr->rcol[2]);
		++nsent;
	}
	return(nsent);
}
...
sample_sdf(BSDFDAT *ndp, int sflags)
{
	...
	FVECT		vjit;
#if defined(_WIN32) || defined(_WIN64)
	unsigned long long rng_state_start = 0;
	unsigned long long rng_state_hemi = 0;
#endif
	...
	if (dfp == NULL)			/* no specular component? */
		return(0);
#if defined(_WIN32) || defined(_WIN64)
	rng_state_start = randstate();
#endif
	if (RCO_BSDF_INSTRUMENT)
		fprintf(stderr,
			"RCO_BSDF_CPU_INSTRUMENT sample_sdf_start "
			"flags=%c hasthru=%d maxHemi=%.9g ncomp=%d "
			"srmin=%.9g srmax=%.9g vray=(%.6g,%.6g,%.6g)"
#if defined(_WIN32) || defined(_WIN64)
			" rng_state=%llu"
#endif
			"\n",
			sflags == SDsampSpT ? 'T' : 'R', hasthru,
			dfp->maxHemi, dfp->ncomp, ndp->sr_vpsa[0],
			ndp->sr_vpsa[1],
			ndp->vray[0], ndp->vray[1], ndp->vray[2]
#if defined(_WIN32) || defined(_WIN64)
			, rng_state_start
...
#if defined(_WIN32) || defined(_WIN64)
	rng_state_hemi = randstate();
#endif
	if (RCO_BSDF_INSTRUMENT)
		fprintf(stderr,
			"RCO_BSDF_CPU_INSTRUMENT sample_sdf_hemi "
			"flags=%c hasthru=%d thruY=%.9g hemi_minus_thru=%.9g "
			"threshold=%.9g maxHemi=%.9g "
			"vjit=(%.9g,%.9g,%.9g)"
#if defined(_WIN32) || defined(_WIN64)
			" rng_state=%llu"
#endif
			"\n",
			sflags == SDsampSpT ? 'T' : 'R', hasthru,
			hasthru ? pbright(ndp->cthru) : 0.0,
			b, specthresh, dfp->maxHemi,
			vjit[0], vjit[1], vjit[2]
#if defined(_WIN32) || defined(_WIN64)
			, rng_state_hemi
#endif
			);
	if (b <= specthresh+FTINY) {		/* below sampling threshold? */
		if (b > FTINY) {		/* XXX no color from BSDF */
			...
			if (RCO_BSDF_INSTRUMENT)
				fprintf(stderr,
					"RCO_BSDF_CPU_INSTRUMENT sample_sdf_unsampled "
					"flags=%c unsc=(%.9g,%.9g,%.9g)\n",
					sflags == SDsampSpT ? 'T' : 'R',
					unsc[0], unsc[1], unsc[2]);
		}
		return(ntotal);
	}					/* else sample specular */
	/*
	 * Use a stable material object index for the sampling dimension.  The
	 * former pointer salt made deterministic BSDF sampling depend on process
	 * address layout, which prevents exact replay across rcontrib front ends.
	 */
	dimlist[ndims_inc()] = (int)objndx(ndp->mp);
	ndims_inc();
	for (n = dfp->ncomp; n--; ) {		/* loop over components */
		dimlist[ndims-1] = n + 9438;
		ntotal += sample_sdcomp(ndp, &dfp->comp[n],
					sflags==SDsampSpT, n);
	}
	dec_ndims(); dec_ndims();
	return(ntotal);
...
m_bsdf(OBJREC *m, RAY *r)
{
	...
	if ((nd.sd->tf == NULL) & (nd.sd->tb == NULL) &&
		...
		if (RCO_BSDF_INSTRUMENT)
			fprintf(stderr,
				"RCO_BSDF_CPU_INSTRUMENT m_bsdf_direct_call "
				"callback=dir_brdf thick=%.9g tdiffY=%.9g\n",
				nd.thick, sintens(nd.tdiff));
		direct(r, dir_brdf, &nd);	/* reflection only */
	} else if (nd.thick == 0) {
		if (RCO_BSDF_INSTRUMENT)
			fprintf(stderr,
				"RCO_BSDF_CPU_INSTRUMENT m_bsdf_direct_call "
				"callback=dir_bsdf thick=%.9g tdiffY=%.9g\n",
				nd.thick, sintens(nd.tdiff));
		...
		if (RCO_BSDF_INSTRUMENT)
			fprintf(stderr,
				"RCO_BSDF_CPU_INSTRUMENT m_bsdf_direct_call "
				"callback=dir_btdf thick=%.9g tdiffY=%.9g\n",
				nd.thick, sintens(nd.tdiff));
