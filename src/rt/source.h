...
typedef struct {
	...
	int  last;				/* last source number to sample */
...
#define initsrcindex(s)	((s)->sn = (s)->sp = -1, (s)->last = nsources-1, (s)->np = 0)
#define inittargetsrcindex(s,n)	((s)->sn = (n)-1, (s)->sp = -1, (s)->last = (n), (s)->np = 0)
...
extern int  direct_specular_only;	/* suppress diffuse terms in direct() callbacks */
extern int  direct_component_tracking; /* retain total and no-diffuse coefficients */
