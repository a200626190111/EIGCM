...
/* Must match the private protocol in rcontrib_sparse.c. */
#define RCW_DENSE       0
#define RCW_SPARSE      1
...
queue_results(int k)
{
	...
	for (j = 0; j < nmods; j++) {
		...
		if (getbinary(&mode, sizeof(int), 1, kida[k].infp) != 1)
			error(SYSTEM, "read error from render process");
		if (mode == RCW_DENSE) {
			if (getbinary(mp->cbin, DCOLORSIZ, mp->nbins,
						kida[k].infp) != mp->nbins)
				error(SYSTEM, "read error from render process");
		} else if (mode == RCW_SPARSE) {
			int	nactive, a;

			memset(mp->cbin, 0, DCOLORSIZ*mp->nbins);
			if (getbinary(&nactive, sizeof(int), 1, kida[k].infp) != 1 ||
					nactive < 0 || nactive > mp->nbins)
				error(SYSTEM, "bad sparse record from render process");
			for (a = 0; a < nactive; a++) {
				int	bn;
				if (getbinary(&bn, sizeof(int), 1, kida[k].infp) != 1 ||
						bn < 0 || bn >= mp->nbins)
					error(SYSTEM, "bad sparse bin from render process");
				if (getbinary(mcbin(mp, bn), sizeof(DCOLORV),
						NCSAMP, kida[k].infp) != NCSAMP)
					error(SYSTEM, "read error from render process");
			}
		} else
			error(SYSTEM, "bad record mode from render process");
	}
	...
	for (;;) {
		...
		firstfree = -1;
		{
			...
			for (i = nchild; i--; ) {
				...
				if (avail > 0) {
					/*
					 * Do not wait for a complete dense contribution record to
					 * accumulate inside the pipe.  With many bins, a single
					 * worker record may be larger than the Windows pipe buffer;
					 * waiting for all bytes here can deadlock because the worker
					 * cannot finish writing until the parent starts reading.
					 * queue_results() performs the blocking full-record read.
					 */
					queue_results(i);
					if (firstfree < 0)
						firstfree = i;
...
feeder_loop()
{
	...
	orgdir = (FVECT *)malloc(sizeof(FVECT)*2*MAXIQ);
	if (orgdir == NULL)
		error(SYSTEM, "out of memory in feeder_loop()");
