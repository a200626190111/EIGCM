...
#ifndef  RCO_TRANS_GAUSS_INSTRUMENT
#define  RCO_TRANS_GAUSS_INSTRUMENT (getenv("RCO_TRANS_GAUSS_INSTRUMENT") != NULL)
#endif
...
)
{
	...
	if ((np->specfl & (SP_REFL|SP_RBLT)) == SP_REFL &&
		...
		for (nstaken = ntrials = 0; nstaken < nstarget &&
			...
			double rco_dbg_draw, rco_dbg_slope, rco_dbg_dot;
			int rco_dbg_dimhash, rco_dbg_ndims;
			rco_dbg_ndims = ndims;
			rco_dbg_dimhash = ilhash(dimlist, ndims);
			if (ntrials)
				d = frandom();
			else
				d = urand(rco_dbg_dimhash+samplendx);
			rco_dbg_draw = d;
			multisamp(rv, 2, d);
			d = 2.0*PI * rv[0];
			cosp = tcos(d);
			...
			rco_dbg_dot = DOT(sr.rdir, np->rp->ron);
			if (RCO_TRANS_GAUSS_INSTRUMENT && np->mp != NULL &&
					np->mp->otype == MAT_TRANS)
				fprintf(stderr,
					"RCO_TRANS_GAUSS_CPU refl parent_rno=%lu "
					"sr_rno=%lu samplendx=%d nstarget=%d "
					"trial=%d taken=%d draw=%.17g "
					"dimhash=%d ndims=%d "
					"dim0=%d dim1=%d dim2=%d "
					"rv=(%.17g,%.17g) slope=%.17g "
					"parent_obj=%d parent_otype=%d parent_oname=%s "
					"parent_rorg=(%.17g,%.17g,%.17g) "
					"parent_rop=(%.17g,%.17g,%.17g) "
					"parent_rot=%.17g parent_rdir=(%.17g,%.17g,%.17g) "
					"sr_rorg=(%.17g,%.17g,%.17g) "
					"raw_dir=(%.17g,%.17g,%.17g) "
					"rawdot=%.17g accept=%d "
					"rcoef=(%.17g,%.17g,%.17g) "
					"rweight=%.17g\n",
					(unsigned long)np->rp->rno,
					(unsigned long)sr.rno, samplendx,
					nstarget, ntrials, nstaken,
					rco_dbg_draw, rco_dbg_dimhash,
					rco_dbg_ndims,
					rco_dbg_ndims > 0 ? dimlist[0] : 0,
					rco_dbg_ndims > 1 ? dimlist[1] : 0,
					rco_dbg_ndims > 2 ? dimlist[2] : 0,
					rv[0], rv[1],
					rco_dbg_slope,
					np->rp->ro != NULL ? objndx(np->rp->ro) : -1,
					np->rp->ro != NULL ? np->rp->ro->otype : -1,
					np->rp->ro != NULL ? np->rp->ro->oname : "(null)",
					np->rp->rorg[0], np->rp->rorg[1],
					np->rp->rorg[2], np->rp->rop[0],
					np->rp->rop[1], np->rp->rop[2],
					np->rp->rot, np->rp->rdir[0],
					np->rp->rdir[1], np->rp->rdir[2],
					sr.rorg[0], sr.rorg[1], sr.rorg[2],
					sr.rdir[0],
					sr.rdir[1], sr.rdir[2], rco_dbg_dot,
					rco_dbg_dot > FTINY,
					sr.rcoef[0], sr.rcoef[1],
					sr.rcoef[2], sr.rweight);
