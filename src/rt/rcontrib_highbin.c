...
/*
 * High-bin optimization: track bins touched since the previous output so
 * clearing the accumulation buffer is proportional to the number of active
 * bins instead of the total number of bins.  This intentionally lives here
 * rather than in MODCONT so the public structure in rcontrib.h need not
 * change.
 */
typedef struct dirtymod {
	MODCONT		*mp;
	unsigned char	*dirty;
	int		*bins;
	int		nbins;
	int		ndirty;
	struct dirtymod	*next;
} DIRTYMOD;

static DIRTYMOD	*dirtymods = NULL;

static DIRTYMOD *
dirty_find(MODCONT *mp, int create)
{
	DIRTYMOD	*dm;

	for (dm = dirtymods; dm != NULL; dm = dm->next)
		if (dm->mp == mp)
			return(dm);
	if (!create)
		return(NULL);
	dm = (DIRTYMOD *)calloc(1, sizeof(DIRTYMOD));
	if (dm == NULL)
		error(SYSTEM, "out of memory in dirty_find()");
	dm->dirty = (unsigned char *)calloc(mp->nbins, sizeof(unsigned char));
	dm->bins = (int *)malloc(sizeof(int)*mp->nbins);
	if (dm->dirty == NULL || dm->bins == NULL)
		error(SYSTEM, "out of memory in dirty_find()");
	dm->mp = mp;
	dm->nbins = mp->nbins;
	dm->next = dirtymods;
...
/*
 * Dirty-bin tracking is only useful when output/clearing happens very often.
 * In the normal multi-process parent-controlled case with -c > 1, workers set
 * accumulate to 0 and output one dense record per assigned batch; marking every
 * ray then becomes pure overhead.  Keep it disabled for those batch modes.
 */
#define DIRTY_ACCUM_MAX	64
#define DIRTY_NBINS_MIN	4096

static int
dirty_enabled(MODCONT *mp)
{
	return (!recover) & (accumulate > 0) &
			(accumulate <= DIRTY_ACCUM_MAX) &
			(mp->nbins >= DIRTY_NBINS_MIN);
}

static void
dirty_mark(MODCONT *mp, int bn)
{
	DIRTYMOD	*dm;
	...
	if (!dm->dirty[bn]) {
		dm->dirty[bn] = 1;
		dm->bins[dm->ndirty++] = bn;
	}
}

static void
dirty_reset(DIRTYMOD *dm)
{
	int	k;
	...
	if (dm == NULL)
		return;
	for (k = dm->ndirty; k--; )
		dm->dirty[dm->bins[k]] = 0;
	dm->ndirty = 0;
}

static void
clear_mod_contrib(MODCONT *mp)
{
	DIRTYMOD	*dm = dirty_find(mp, 0);
	int	k;

	/*
	 * If dirty tracking was not enabled for this accumulation mode, use the
	 * original dense clear path.  This is faster for large -c batches because
	 * clearing happens only once per batch, while dirty marking would run once
	 * per ray hit.
	 */
	if (!dirty_enabled(mp) || dm == NULL || dm->ndirty <= 0 ||
			dm->ndirty >= mp->nbins/4) {
		memset(mp->cbin, 0, DCOLORSIZ*mp->nbins);
		dirty_reset(dm);
		return;
	}
	for (k = dm->ndirty; k--; )
		memset(mcbin(mp, dm->bins[k]), 0, DCOLORSIZ);
	dirty_reset(dm);
}
