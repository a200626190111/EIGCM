...
#define  SPECULAR	(RSPECULAR|TSPECULAR)
				...
				/* recorded optical path events (4 bits/event) */
#define  RPE_MIRROR	1		/* selected mirror reflection */
#define  RPE_BSDF_REFL	2		/* BSDF reflection */
#define  RPE_BSDF_TRANS	3		/* BSDF transmission */
#define  RPE_OTHER_SPEC	4		/* other redirected/specular event */
#define  RPE_DIFFUSE	5		/* diffuse ambient event */
...
typedef struct ray {
	...
	int	rtarget;	/* source exclusively evaluated for this ray tree */
	unsigned int rpath_orders; /* allowed specular path orders for this ray tree */
	unsigned int rpath_depth; /* accepted target reflections so far */
	unsigned int rpath_signature; /* ordered optical events, 4 bits each */
	unsigned short rpath_event_count; /* number of recorded optical events */
	unsigned short rpath_mirror_count; /* selected mirror reflections */
	unsigned short rpath_bsdf_count; /* BSDF reflection/transmission events */
	unsigned short rpath_other_count; /* other redirected/specular events */
	unsigned short rpath_diffuse_count; /* diffuse ambient events */
	unsigned short rpath_redirect_count; /* events that changed ray direction */
	SCOLOR	rpath_direct_ratio; /* no-diffuse / total coefficient ratio */
...
extern int	(*ray_spawn_check)(RAY *, const RAY *);
