...
int	(*ray_spawn_check)(RAY *, const RAY *) = NULL;
...
)
{
	...
	if ((r->parent = ro) == NULL) {		/* primary ray */
		...
		r->rpath_depth = 0;
		r->rpath_signature = 0;
		r->rpath_event_count = 0;
		r->rpath_mirror_count = 0;
		r->rpath_bsdf_count = 0;
		r->rpath_other_count = 0;
		r->rpath_diffuse_count = 0;
		r->rpath_redirect_count = 0;
		setscolor(r->rpath_direct_ratio, 1., 1., 1.);
		r->clipset = NULL;
		r->revf = raytrace;
		...
		r->rlvl = ro->rlvl;
		r->rsrc = ro->rsrc;
		r->rtarget = ro->rtarget;
		r->rpath_orders = ro->rpath_orders;
		r->rpath_depth = ro->rpath_depth;
		r->rpath_signature = ro->rpath_signature;
		r->rpath_event_count = ro->rpath_event_count;
		r->rpath_mirror_count = ro->rpath_mirror_count;
		r->rpath_bsdf_count = ro->rpath_bsdf_count;
		r->rpath_other_count = ro->rpath_other_count;
		r->rpath_diffuse_count = ro->rpath_diffuse_count;
		r->rpath_redirect_count = ro->rpath_redirect_count;
		copyscolor(r->rpath_direct_ratio, ro->rpath_direct_ratio);
		if (rt & RAYREFL) {
			r->rlvl++;
	...
	rayclear(r);
	if (ro != NULL && ray_spawn_check != NULL &&
			!(*ray_spawn_check)(r, ro)) {
		r->rweight = 0.0;
		return(-1);
	}
	if (RCO_BSDF_INSTRUMENT)
		fprintf(stderr,
			"RCO_BSDF_CPU_INSTRUMENT rayorigin rno=%lu rt=%d "
			"parent_rno=%ld crtype=%u rweight=%.9g "
			"coef=(%.9g,%.9g,%.9g)\n",
			(unsigned long)r->rno, rt,
			ro != NULL ? (long)ro->rno : -1L,
			(unsigned int)r->crtype, r->rweight,
