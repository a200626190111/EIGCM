...
getostream(const char *ospec, const char *mname, int bn, int noopen)
{
	...
	char			info_static[1024];
	char			*info = info_static;
	int			ofl;
	char			oname[1024];
	LUENT			*lep;
	STREAMOUT		*sop;
...
#endif
			if (header) {
				/* PATH_COMPONENT_BINS contains one entry per modifier.
				 * Annual source lists can contain thousands of modifiers,
				 * so the legacy fixed header buffer is not large enough. */
				if (rc_dual_path_output && nmods > 0) {
					size_t infosiz = sizeof(info_static) +
							(size_t)nmods * 24;
					info = (char *)malloc(infosiz);
					if (info == NULL)
						error(SYSTEM,
							"out of memory building path-component header");
					info[0] = '\0';
				}
				cp = info;
				if (yres > 0) {
				...
				if (rc_dual_path_output) {
					int	i;
					sprintf(cp, "PATH_COMPONENTS=total,direct\n");
					while (*cp) ++cp;
					sprintf(cp, "PATH_COMPONENT_BINS=");
					while (*cp) ++cp;
					for (i = 0; i < nmods; i++) {
						MODCONT *mp = (MODCONT *)lu_find(&modconttab,
								modname[i])->data;
						sprintf(cp, "%s%d", i ? "," : "", mp->base_nbins);
						while (*cp) ++cp;
					}
					sprintf(cp, "\n");
				}
