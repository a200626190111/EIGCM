...
/*
 * Experimental worker-to-parent sparse contribution protocol.
 * Final user output remains unchanged; this only changes internal child stdout
 * when nproc > 1.  A worker sends each modifier record either as the original
 * dense cbin array, or as a sparse list of non-zero bins if that is smaller.
 */
#define RCW_DENSE       0
#define RCW_SPARSE      1
#define RCW_SPARSE_NBINS_MIN 4096
#define RCW_SPARSE_MARGIN_NUM 9
#define RCW_SPARSE_MARGIN_DEN 10

static int
bin_nonzero(const DCOLORV *dp)
{
	int i;
	for (i = NCSAMP; i--; )
		if (dp[i] != 0.0)
...
static int
count_nonzero_bins(const MODCONT *mp)
{
	int j, nactive = 0;
	for (j = 0; j < mp->nbins; j++)
		if (bin_nonzero(mcbin((MODCONT *)mp, j)))
			++nactive;
	return(nactive);
}

/* Write one modifier record for parent queue_results(). */
static void
worker_output_modifier(MODCONT *mp)
{
	int mode = RCW_DENSE;
	int nactive = -1;
	int j;

	if (mp->nbins >= RCW_SPARSE_NBINS_MIN) {
		size_t dense_bytes, sparse_bytes;
		nactive = count_nonzero_bins(mp);
		dense_bytes = (size_t)mp->nbins * (size_t)DCOLORSIZ;
		sparse_bytes = 2*sizeof(int) +
				(size_t)nactive * (sizeof(int) + (size_t)DCOLORSIZ);
		/* Use sparse only when it is clearly smaller, leaving a margin for
		 * the extra decode work in the parent. */
		if (sparse_bytes * RCW_SPARSE_MARGIN_DEN <
				dense_bytes * RCW_SPARSE_MARGIN_NUM)
			mode = RCW_SPARSE;
	}

	putbinary(&mode, sizeof(int), 1, stdout);

	if (mode == RCW_DENSE) {
		putbinary(mp->cbin, DCOLORSIZ, mp->nbins, stdout);
		if (ferror(stdout))
	...
	if (nactive < 0)
		nactive = count_nonzero_bins(mp);
	putbinary(&nactive, sizeof(int), 1, stdout);
	for (j = 0; j < mp->nbins; j++) {
		const DCOLORV *dp = mcbin(mp, j);
		if (!bin_nonzero(dp))
			continue;
		putbinary(&j, sizeof(int), 1, stdout);
		putbinary(dp, sizeof(DCOLORV), NCSAMP, stdout);
	}
	if (ferror(stdout))
		error(SYSTEM, "pipe write error");
}
...
done_contrib(void)
{
	...
	if (nchild < 0) {			/* worker process: private protocol */
		for (i = 0; i < nmods; i++) {
			mp = (MODCONT *)lu_find(&modconttab,modname[i])->data;
			worker_output_modifier(mp);
			memset(mp->cbin, 0, DCOLORSIZ*mp->nbins);
		}
		fflush(stdout);
	} else {				/* normal single-process user output */
