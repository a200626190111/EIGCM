...
)
{
	...
	if (rayorigin(&ar, hp->atyp, hp->rp, ar.rcoef) < 0) {
		if (RCO_BSDF_INSTRUMENT)
			fprintf(stderr,
				"RCO_BSDF_CPU_INSTRUMENT ambsample_origin_fail "
				"atyp=%d ns=%d i=%d j=%d n=%d rno=%lu "
				"samplendx=%lu coef=(%.9g,%.9g,%.9g)\n",
				hp->atyp, hp->ns, i, j, n,
				(unsigned long)hp->rp->rno, samplendx,
				ar.rcoef[0], ar.rcoef[1], ar.rcoef[2]);
		return(0);
	}
	if (ambacc > FTINY) {
	...
	while (trade_ok-- && ambcollision(hp, i, j, spt)) {
		...
		if (RCO_BSDF_INSTRUMENT)
			fprintf(stderr,
				"RCO_BSDF_CPU_INSTRUMENT ambsample_collision_step "
				"atyp=%d ns=%d i=%d j=%d n=%d rno=%lu "
				"samplendx=%lu step=%d trade_ok=%d "
				"ss_before=(%.9g,%.9g) rng_state=%llu\n",
				hp->atyp, hp->ns, i, j, n,
				(unsigned long)hp->rp->rno, samplendx,
				collisions, trade_ok, ss[0], ss[1],
				randstate());
#endif
		if (trade_ok) {
			trade_patchsamp(ss);
		...
		if (RCO_BSDF_INSTRUMENT)
			fprintf(stderr,
				"RCO_BSDF_CPU_INSTRUMENT ambsample_collision_step_done "
				"atyp=%d ns=%d i=%d j=%d n=%d rno=%lu "
				"samplendx=%lu step=%d ss_after=(%.9g,%.9g) "
				"rng_state=%llu\n",
				hp->atyp, hp->ns, i, j, n,
				(unsigned long)hp->rp->rno, samplendx,
				collisions, ss[0], ss[1], randstate());
#endif
		square2disk(spt, (j+ss[1])/hp->ns, (i+ss[0])/hp->ns);
	}
	if (RCO_BSDF_INSTRUMENT && collisions)
		fprintf(stderr,
			"RCO_BSDF_CPU_INSTRUMENT ambsample_collision "
			"atyp=%d ns=%d i=%d j=%d n=%d rno=%lu "
			"samplendx=%lu collisions=%d ss=(%.9g,%.9g) "
			"spt=(%.9g,%.9g)\n",
			hp->atyp, hp->ns, i, j, n,
			(unsigned long)hp->rp->rno, samplendx, collisions,
			ss[0], ss[1], spt[0], spt[1]);
	...
	if (RCO_BSDF_INSTRUMENT)
		fprintf(stderr,
			"RCO_BSDF_CPU_INSTRUMENT ambsample "
			"atyp=%d ns=%d i=%d j=%d n=%d rno=%lu samplendx=%lu "
			"ss=(%.9g,%.9g) spt=(%.9g,%.9g) "
			"rdir=(%.6g,%.6g,%.6g) "
			"coef=(%.9g,%.9g,%.9g) hit_mod=%d rsrc=%d crtype=%u "
			"rcol=(%.9g,%.9g,%.9g)\n",
			hp->atyp, hp->ns, i, j, n,
			(unsigned long)hp->rp->rno, samplendx,
			ss[0], ss[1], spt[0], spt[1],
			ar.rdir[0], ar.rdir[1], ar.rdir[2],
			ar.rcoef[0], ar.rcoef[1], ar.rcoef[2],
			ar.ro != NULL ? ar.ro->omod : -1, ar.rsrc,
			(unsigned int)ar.crtype,
			ar.rcol[0], ar.rcol[1], ar.rcol[2]);
	dec_ndims();
	zd = raydistance(&ar);
	if (zd <= FTINY)
...
)
{
	...
	if (RCO_BSDF_INSTRUMENT)
		fprintf(stderr,
			"RCO_BSDF_CPU_INSTRUMENT ambbasis_start "
			"atyp=%d ns=%d rno=%lu samplendx=%lu "
			"onrm=(%.9g,%.9g,%.9g) rng_state=%llu\n",
			hp->atyp, hp->ns, (unsigned long)hp->rp->rno,
			samplendx, hp->onrm[0], hp->onrm[1], hp->onrm[2],
			randstate());
#endif
	...
	if (RCO_BSDF_INSTRUMENT)
		fprintf(stderr,
			"RCO_BSDF_CPU_INSTRUMENT ambbasis_done "
			"atyp=%d ns=%d rno=%lu samplendx=%lu "
			"ux=(%.9g,%.9g,%.9g) uy=(%.9g,%.9g,%.9g) "
			"rng_state=%llu\n",
			hp->atyp, hp->ns, (unsigned long)hp->rp->rno,
			samplendx, hp->ux[0], hp->ux[1], hp->ux[2],
			hp->uy[0], hp->uy[1], hp->uy[2], randstate());
#endif
					/* sample divisions */
	for (i = hp->ns; i--; )
	    for (j = hp->ns; j--; )
