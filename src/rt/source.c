...
typedef struct {
	...
	SCOLOR  direct_coef;	/* coefficient without diffuse direct terms */
...
int  direct_specular_only = 0;		/* direct() callbacks omit diffuse terms */
int  direct_component_tracking = 0;	/* evaluate total and no-diffuse coefficients */


static void
set_direct_component_ratio(RAY *ray, const CONTRIB *scp)
{
	int	i;

	for (i = 0; i < NCSAMP; i++) {
		double	ratio = 0.;
		if (fabs(scp->coef[i]) > FTINY)
			ratio = scp->direct_coef[i] / scp->coef[i];
		ray->rpath_direct_ratio[i] *= ratio;
	}
}
...
)
{
	...
	if (r->rsrc >= 0) {		/* check only one if aimed */
		first = last = r->rsrc;
	} else if (r->rtarget >= 0 && r->rtarget < nsources) {
		first = last = r->rtarget;
...
)
{
	...
	if (RCO_BSDF_INSTRUMENT)
		fprintf(stderr,
			"RCO_BSDF_CPU_INSTRUMENT direct_enter "
			"maxcntr=%d rweight=%.9g parentY=%.9g "
			"obj=%s org=(%.9g,%.9g,%.9g) dir=(%.9g,%.9g,%.9g) "
			"hit=(%.9g,%.9g,%.9g) normal=(%.9g,%.9g,%.9g) "
			"rod=%.9g rot=%.9g crtype=%u\n",
			maxcntr, r->rweight, pbright(r->rcol),
			r->ro != NULL && r->ro->oname != NULL ? r->ro->oname : "(none)",
			r->rorg[0], r->rorg[1], r->rorg[2],
			r->rdir[0], r->rdir[1], r->rdir[2],
			r->rop[0], r->rop[1], r->rop[2],
			r->ron[0], r->ron[1], r->ron[2],
			r->rod, r->rot, r->crtype);
	if (maxcntr <= 0) {
		if (RCO_BSDF_INSTRUMENT)
			fprintf(stderr,
				"RCO_BSDF_CPU_INSTRUMENT direct_no_sources\n");
		return;		/* no direct?! */
	}

	if (r->rtarget >= 0 && r->rtarget < nsources)
		inittargetsrcindex(&si, r->rtarget);
	else
	...
	for (sn = 0; srcray(&sr, r, &si); sn++) {
...
#endif
						/* compute coefficient */
		if (direct_component_tracking) {
			int	saved_mode = direct_specular_only;
			direct_specular_only = 0;
			(*f)(scp->coef, p, sr.rdir, si.dom);
			direct_specular_only = 1;
			(*f)(scp->direct_coef, p, sr.rdir, si.dom);
			direct_specular_only = saved_mode;
		} else {
			(*f)(scp->coef, p, sr.rdir, si.dom);
			copyscolor(scp->direct_coef, scp->coef);
		}
		...
		if (cntord[sn].brt <= 0.0) {
			if (RCO_BSDF_INSTRUMENT)
				fprintf(stderr,
					...
					"direct_candidate_zero "
					"sn=%d sno=%d si_np=%d si_sp=%d "
					"dom=%.9g dir=(%.6g,%.6g,%.6g) "
					"coef=(%.9g,%.9g,%.9g)\n",
					sn, scp->sno, si.np, si.sp, si.dom,
					sr.rdir[0], sr.rdir[1], sr.rdir[2],
					scp->coef[0], scp->coef[1],
					scp->coef[2]);
			continue;
		}
		...
		if (direct_component_tracking)
			set_direct_component_ratio(&sr, scp);
						/* compute potential */
		sr.revf = srcvalue;
		rayvalue(&sr);
		...
		if (RCO_BSDF_INSTRUMENT)
			fprintf(stderr,
				"RCO_BSDF_CPU_INSTRUMENT direct_candidate "
				"sn=%d sno=%d si_np=%d si_sp=%d dom=%.9g "
				"dir=(%.6g,%.6g,%.6g) "
				"coef=(%.9g,%.9g,%.9g) "
				"val=(%.9g,%.9g,%.9g) brt=%.9g\n",
				sn, scp->sno, si.np, si.sp, si.dom,
				sr.rdir[0], sr.rdir[1], sr.rdir[2],
				scp->coef[0], scp->coef[1], scp->coef[2],
				scp->val[0], scp->val[1], scp->val[2],
				cntord[sn].brt);
	}
						/* sort contributions */
	...
	if (ncnts == 0) {
		if (RCO_BSDF_INSTRUMENT)
			fprintf(stderr,
				"RCO_BSDF_CPU_INSTRUMENT "
				"direct_no_contrib srcray_count=%d\n",
				sn);
	...
	if (RCO_BSDF_INSTRUMENT)
		fprintf(stderr,
			"RCO_BSDF_CPU_INSTRUMENT direct_sort "
			"ncnts=%d nshadcheck=%d shadthresh=%.9g "
			"ourthresh=%.9g rweight=%.9g parentY=%.9g\n",
			ncnts, nshadcheck, shadthresh, ourthresh,
			r->rweight, pbright(r->rcol));
						/* test for shadows */
	for (nhits = 0, hwt = 0.0, sn = 0; sn < ncnts;
		...
		if (sn >= MINSHADCNT &&
			...
			if (RCO_BSDF_INSTRUMENT)
				fprintf(stderr,
					"RCO_BSDF_CPU_INSTRUMENT direct_break "
					"rank=%d ncnts=%d window_brt=%.9g "
					"threshold=%.9g\n",
					sn, ncnts,
					sn+nshadcheck>=ncnts ? cntord[sn].brt :
					cntord[sn].brt-cntord[sn+nshadcheck].brt,
		...
		if (RCO_BSDF_INSTRUMENT)
			fprintf(stderr,
				"RCO_BSDF_CPU_INSTRUMENT direct_shadow_test "
				"rank=%d sndx=%d sno=%d brt_tail=%.9g "
				"dir=(%.6g,%.6g,%.6g) "
				"coef=(%.9g,%.9g,%.9g)\n",
				sn, cntord[sn].sndx, scp->sno, cntord[sn].brt,
				scp->dir[0], scp->dir[1], scp->dir[2],
				scp->coef[0], scp->coef[1], scp->coef[2]);
						/* test for hit */
		...
		if (localhit(&sr, &thescene) &&
			...
			if (scolor_mean(sr.rcol) <= FTINY) {
				...
				if (RCO_BSDF_INSTRUMENT)
					fprintf(stderr,
						"RCO_BSDF_CPU_INSTRUMENT "
						"direct_shadow_miss rank=%d sno=%d\n",
						sn, scp->sno);
				continue;	/* missed! */
			}
			rayparticipate(&sr);
		...
		if (RCO_BSDF_INSTRUMENT)
			fprintf(stderr,
				"RCO_BSDF_CPU_INSTRUMENT direct_shadow_add "
				"rank=%d sno=%d val=(%.9g,%.9g,%.9g) "
				"parent=(%.9g,%.9g,%.9g)\n",
				sn, scp->sno, scp->val[0], scp->val[1],
				scp->val[2], r->rcol[0], r->rcol[1],
				r->rcol[2]);
		nhits++;
	...
	for ( ; sn < ncnts; sn++) {
		...
		if (RCO_BSDF_INSTRUMENT)
			fprintf(stderr,
				"RCO_BSDF_CPU_INSTRUMENT direct_tail_add "
				"rank=%d sno=%d prob=%.9g val=(%.9g,%.9g,%.9g) "
				"parent=(%.9g,%.9g,%.9g)\n",
				sn, scp->sno, prob, scp->val[0], scp->val[1],
				scp->val[2], r->rcol[0], r->rcol[1],
				r->rcol[2]);
	}
}
