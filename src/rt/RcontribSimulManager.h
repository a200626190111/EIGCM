...
/// Optional terminal-ray predicate for in-process contribution filtering
typedef int	RcontribRayFilter(const RAY *ray, void *data);
...
protected:
	...
	bool			targetOnlyOutput; // one NCSAMP value per targeted ray
	bool			sourceComponentOutput; // total plus zero-redirect source bins
	bool			sourceVisibilityOutput; // zero-redirect source hit fraction
	RcontribRayFilter *	rayFilter;     // optional terminal path predicate
	void *			rayFilterData; // caller-owned predicate context
...
public:
	...
	uint32			accum;		// # rays to accumulate per record
				RcontribSimulManager(const char *octn = NULL)
					...
					targetOnlyOutput = false;
					sourceComponentOutput = false;
					sourceVisibilityOutput = false;
					rayFilter = NULL; rayFilterData = NULL;
					outOp = RCOnew;
					cdsF = &defDataShare;
					xres = yres = 0;
					accum = 1;
	...
	int			GetFormat(int *siz = NULL) const {
				...
				/// Store only the modifier associated with targetSource per row
	bool			SetTargetOnlyOutput(bool enable = true) {
					if (outList || Ready()) return false;
					targetOnlyOutput = enable;
					return true;
				}
				/// Store total and zero-redirect physical-source components
	bool			SetSourceComponentOutput(bool enable = true) {
					if (outList || Ready() || targetOnlyOutput) return false;
					sourceComponentOutput = enable;
					return true;
				}
				/// Store the fraction of zero-redirect physical-source samples
	bool			SetSourceVisibilityOutput(bool enable = true) {
					if (outList || Ready() || targetOnlyOutput) return false;
					sourceVisibilityOutput = enable;
					return true;
				}
				/// Filter terminal contribution rays before accumulation
	bool			SetRayFilter(RcontribRayFilter *filter,
							void *data = NULL) {
					if (outList || Ready()) return false;
					rayFilter = filter;
					rayFilterData = data;
	...
	int			ComputeRecord(const FVECT orig_direc[]);
				/// Compute record while evaluating one source exclusively
	int			ComputeRecordTarget(const FVECT orig_direc[],
						int targetSource,
						unsigned int pathOrders = 0);
