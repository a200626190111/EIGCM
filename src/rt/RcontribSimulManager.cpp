...
struct RcontribMod {
	...
	int			base_nbins;	// directional bins before components
	int			ncomponents;	// output components per directional bin
...
struct RowAssignment {
	...
	int32			targetSource;	// exclusive source for this ray tree
	uint32			pathOrders;	// allowed specular reflection orders
};
		...
		int ncomponents)
{
	...
	const int base_nbins = ncbins;
	const int total_bins = base_nbins*ncomponents;
	
	RcontribMod *	mp = (RcontribMod *)ecalloc(1, sizeof(RcontribMod) +
						sizeof(DCOLORV)*(NCSAMP*total_bins-1) +
						strlen(prms)+1);
	...
	mp->params = strcpy((char *)(mp->cbin + total_bins*NCSAMP), prms);
	mp->base_nbins = base_nbins;
	mp->ncomponents = ncomponents;
	mp->nbins = total_bins;
	return mp;
}
...
RcontribSimulManager::RctCall(RAY *r, void *cd)
{
	...
	if (rcp->rayFilter && !(*rcp->rayFilter)(r, rcp->rayFilterData))
		return 0;		// rejected by caller's optical-path predicate
	if (r->rpath_orders) {
		if (!r->rpath_depth || r->rpath_depth > 2)
			return 0;
		if (!(r->rpath_orders & (1u << (r->rpath_depth-1))))
			return 0;
	}
	...
	const bool straightPhysicalSource = r->rsrc >= 0 &&
			r->rpath_redirect_count == 0 &&
			r->rpath_diffuse_count == 0;
	if (rcp->sourceComponentOutput && straightPhysicalSource) {
		dvp = (*mp)[mp->base_nbins+bi];
		for (i = 0; i < NCSAMP; i++)
			*dvp++ += contr[i];
	}
	if (rcp->sourceVisibilityOutput && straightPhysicalSource) {
		const int component = 1 + (rcp->sourceComponentOutput ? 1 : 0);
		dvp = (*mp)[component*mp->base_nbins+bi];
		for (i = 0; i < NCSAMP; i++)
			*dvp++ += 1.0;
	}
	return 1;
				...
				const char *prms, const char *binval, int bincnt)
{
	...
	RcontribMod *	mp = NewRcMod(prms, binval, bincnt,
					1 + (sourceComponentOutput ? 1 : 0) +
					(sourceVisibilityOutput ? 1 : 0));
	if (!mp) return false;
	lp->data = (char *)mp;
	if (targetOnlyOutput) {
		if (mp->nbins != 1 || binval && *binval) {
			error(USER, "target-only output requires exactly one bin");
			return false;
		}
		if (!outList)
			outList = new RcontribOutput(outspec);
		else if (strcmp(outList->GetName(), outspec) || outList->next) {
			error(USER, "target-only modifiers must share one output");
			return false;
		}
		mp->opl = outList;
		mp->coffset = 0;
		if (!outList->rowBytes)
			outList->rowBytes = dsiz;
		return true;
	}
...
{
	return ComputeRecordTarget(orig_direc, -1);
}

// Add a ray/bundle with an optional exclusive source to compute next record
int
RcontribSimulManager::ComputeRecordTarget(const FVECT orig_direc[],
								int targetSource,
								unsigned int pathOrders)
{
	if (!Ready())
		return 0;
	...
	if (nkids > 0) {			// in parent process?
		...
		rass.ac = accum;
		rass.targetSource = targetSource;
		rass.pathOrders = pathOrders;
	...
	if (EnqueueBundle(orig_direc, accum, 0, targetSource, pathOrders) < 0)
		return -1;
	...
	if (targetOnlyOutput && targetSource >= 0) {
		if (targetSource >= nsources || !source[targetSource].so) {
			error(INTERNAL, "invalid target source in ComputeRecordTarget()");
			return -1;
		}
		/* RctCall() may encounter untracked modifiers and grow modLUT while
		 * tracing this record.  LUENT addresses are not stable across such a
		 * growth, so resolve the target after tracing instead of caching the
		 * table entry itself. */
		OBJREC *mat = findmaterial(source[targetSource].so);
		LUENT *lp = mat && mat->oname ? lu_find(&modLUT, mat->oname) : NULL;
		if (!lp || !lp->data || putModContrib(lp, this) < 0) {
			error(INTERNAL, "target source modifier is not tracked");
			return -1;
		}
	} else if (lu_doall(&modLUT, putModContrib, this) < 0)
		return -1;
						// write buffers
	for (op = outList; op; op = op->next)
		op->DoneRow();
...
RcontribSimulManager::RunChild()
{
	...
	while ((nr = read(0, &rass, sizeof(rass))) == sizeof(rass)) {
		...
		if (ComputeRecordTarget(vecList, rass.targetSource,
							rass.pathOrders) <= 0)
			exit(1);
