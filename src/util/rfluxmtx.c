...
char		*binjitter = NULL;
char		*sender_bin_mask = NULL;	/* optional zero-based active bin list */
unsigned long	sender_sample_seed = 0;	/* optional independent sample sequence */
int		sender_sample_seed_set = 0;
uint64_t	sender_sample_start = 0;	/* first sample in nested sequence */
int		sender_sample_start_set = 0;
int		sender_adaptive_accumulate = 0;
...
/* Load a whitespace-separated list of zero-based sender bin indices. */
static unsigned char *
load_sender_bin_mask(const char *fname, int nbins, int *nactive)
{
	FILE		*fp;
	unsigned char	*mask;
	char		line[4096];
	...
	if ((fp = fopen(fname, "r")) == NULL) {
		fprintf(stderr, "%s: cannot open sender bin mask '%s'\n",
				progname, fname);
		return(NULL);
	}
	mask = (unsigned char *)calloc(nbins, sizeof(unsigned char));
	if (mask == NULL) {
		fclose(fp);
		return(NULL);
	}
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
		...
		while (*cp) {
			...
			if (!*cp || *cp == '#')
				break;
			b = strtol(cp, &ep, 10);
			if (ep == cp || b < 0 || b >= nbins) {
				fprintf(stderr,
					"%s: invalid sender bin in %s at line %d\n",
...
/* Preserve row grouping while avoiding scene traversal for inactive bins. */
static int
sample_inactive_bin(FILE *fp)
{
	FVECT	ray[2] = {
		{RFLUX_INACTIVE_X, RFLUX_INACTIVE_Y, RFLUX_INACTIVE_Z},
		{0., 0., 1.}
	};
	int	n = sampcnt;

	while (n--)
		if (putbinary(ray, sizeof(FVECT), 2, fp) != 2)
			return(0);
	return(1);
}

/* Integer hash used to rotate each nested low-discrepancy sequence. */
static uint32_t
sample_hash(uint32_t x)
{
	x ^= x >> 16;
	x *= 0x7feb352dU;
	x ^= x >> 15;
...
/* Radical inverse for a single Halton dimension. */
static double
radical_inverse(uint64_t n, unsigned int base)
{
	double	invb = 1. / (double)base;
	double	place = invb;
	double	value = 0.;

	while (n) {
		value += (double)(n % base) * place;
		n /= base;
		place *= invb;
	}
	return(value);
...
/*
 * Generate a sample that depends only on its absolute index, sender bin,
 * dimension, and seed.  This makes disjoint --sample-start ranges exactly
 * reusable when increasing -c.
 */
static void
nested_sender_sample(double *samp, int ndim, int bin, uint64_t sample_index)
{
	static const unsigned int	prime[] = {2, 3, 5};
	uint32_t	seed = (uint32_t)sender_sample_seed ^
				(uint32_t)((uint64_t)sender_sample_seed >> 32);
	int		d;

	for (d = 0; d < ndim; d++) {
		uint32_t h = sample_hash(seed ^ (uint32_t)(bin + 1)*0x9e3779b9U ^
					(uint32_t)(d + 1)*0x85ebca6bU);
		double rotation = ((double)h + .5) / 4294967296.;

		samp[d] = radical_inverse(sample_index + 1, prime[d]) + rotation;
		samp[d] -= floor(samp[d]);
	}
}

/* Preserve the legacy sequence unless nested sampling was requested. */
static void
sender_multi_sample(double *samp, int ndim, int bin, int remaining)
{
	if (sender_sample_start_set) {
		uint64_t local_index = (uint64_t)(sampcnt - 1 - remaining);
		nested_sender_sample(samp, ndim, bin,
				sender_sample_start + local_index);
	} else
		SDmultiSamp(samp, ndim, (remaining + frandom())/sampcnt);
}
...
sample_origin(PARAMS *p, FVECT orig, const FVECT rdir, double x)
{
	...
	if (tarea < 0/*FTINY * FTINY*/) {	/* wrong side of sender? */
		fprintf(stderr, "%s", "tarea < 0");
		fputs(progname, stderr);
...
sample_uniform(PARAMS *p, int b, FILE *fp)
{
	...
	while (n--) {			/* stratified hemisphere sampling */
		sender_multi_sample(samp3, 3, b, n);
...
sample_klems(PARAMS *p, int b, FILE *fp)
{
	...
	while (n--) {			/* stratified sampling */
		sender_multi_sample(samp2, 2, b, n);
...
main(int argc, char *argv[])
{
	...
	FILE	*rcfp;
	int	nsbins, nactive = 0;
	unsigned char	*active_bins = NULL;
	int	a, i;
					/* set global progname */
	fixargv0(argv[0]);
	...
	for (a = 1; a < argc-2; a++) {
		...
		na = 1;	
		if (argv[a][1] == '-') {
			if (!strcmp(argv[a], "--sender-bin-mask")) {
				if (++a >= argc-2)
					goto userr;
				sender_bin_mask = argv[a];
				continue;
			}
			if (!strcmp(argv[a], "--sample-seed")) {
				char	*ep;

				if (++a >= argc-2)
					goto userr;
				errno = 0;
				sender_sample_seed = strtoul(argv[a], &ep, 10);
				if (errno || ep == argv[a] || *ep)
					goto userr;
				sender_sample_seed_set = 1;
				continue;
			}
			if (!strcmp(argv[a], "--sample-start")) {
				char	*ep;
				...
				if (++a >= argc-2)
					goto userr;
				errno = 0;
				start = strtoull(argv[a], &ep, 10);
				if (errno || ep == argv[a] || *ep)
					goto userr;
				sender_sample_start = (uint64_t)start;
				sender_sample_start_set = 1;
				continue;
			}
			if (!strcmp(argv[a], "--direct-specular-only") ||
					!strcmp(argv[a], "--path-direct-straight-only"))
				na = 1;
			else if (!strcmp(argv[a], "--adaptive-accumulate") ||
					!strcmp(argv[a], "--adaptive-accumulate-report")) {
				na = 2;
				if (!strcmp(argv[a], "--adaptive-accumulate"))
					sender_adaptive_accumulate = 1;
			}
			else if (!strcmp(argv[a], "--path-output") ||
					!strcmp(argv[a], "--path-components") ||
					!strcmp(argv[a], "--path-direct-max-reflections") ||
					!strcmp(argv[a], "--path-direct-max-ambient") ||
					!strcmp(argv[a], "--path-mirror-modifiers") ||
					!strcmp(argv[a], "--path-mirror-orders"))
				na = 2;
			else
				goto userr;
			CHECKARGC(na);
	...
	if (sendfn[0] == '-') {		/* user wants pass-through mode? */
		...
		sendfn = NULL;
		if (sender_sample_start_set) {
			fprintf(stderr, "%s: --sample-start requires sender sampling mode\n",
					progname);
			return(1);
		}
		if (iropt) {
		...
		if (sender_adaptive_accumulate && !sender_sample_start_set) {
			sender_sample_start = 0;
			sender_sample_start_set = 1;
		}
		if (sender_sample_start_set &&
				sender_sample_start > UINT64_MAX - (uint64_t)sampcnt) {
			fprintf(stderr, "%s: nested sample range overflow\n", progname);
			return(1);
		}
	}
	...
	if ((nsbins = prepare_sampler(&curparams)) <= 0)
		return(1);
	if (sender_bin_mask != NULL) {
		active_bins = load_sender_bin_mask(sender_bin_mask, nsbins, &nactive);
		if (active_bins == NULL)
			return(1);
		rcarg[nrcargs++] = "--rflux-inactive-sentinel";
	}
	...
	if (verbose & VERBO) {
		...
		if (curparams.nsurfs > 1)
			...
			fputc('\n', stderr);
		if (active_bins != NULL)
			fprintf(stderr, "%s: active sender directions %d/%d\n",
					progname, nactive, nsbins);
		if (sender_sample_seed_set)
			fprintf(stderr, "%s: sender sample seed %lu\n",
					progname, sender_sample_seed);
		if (sender_sample_start_set)
			fprintf(stderr, "%s: nested sender sample range %llu..%llu\n",
					progname,
					(unsigned long long)sender_sample_start,
					(unsigned long long)(sender_sample_start + sampcnt - 1));
	}
	if (sender_sample_seed_set)
		srandom(sender_sample_seed);
	for (i = 0; i < nsbins; i++)	/* send rcontrib ray samples */
		if (active_bins != NULL && !active_bins[i] ?
				!sample_inactive_bin(rcfp) :
				!(*curparams.sample_basis)(&curparams, i, rcfp))
			return(1);
	free(active_bins);
...
userr:
	...
	fprintf(stderr, "Usage: %s [-v][-bj frac][--sender-bin-mask file][--sample-seed N][--sample-start N][--adaptive-accumulate settings][--path-components total,direct][--path-direct-straight-only][rcontrib options] sender.rad receiver.rad [-i system.oct] [system.rad ..]\n",
				progname);
	return(1);
}
