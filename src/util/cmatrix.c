...
get_cminfo(char *s, void *p)
{
	...
	if (isncomp(s)) {
		ip->ncomp = ncompval(s);
		if ((ip->ncomp != 1) & (ip->ncomp != 3)) {
			ip->err = "unexpected # components (must be 1 or 3)";
			return(-1);
		}
		return(0);
	}
...
static char *
cm_getheader_ncomp(int *dt, int *nr, int *nc, int *swp, int *ncp,
	COLOR scale, FILE *fp)
{
	CMINFO	cmi;
						/* read header */
...
{
	return(cm_getheader_ncomp(dt, nr, nc, swp, NULL, scale, fp));
}
...
cm_load(const char *inspec, int nrows, int ncols, int dtype)
{
	...
	if (!dtype | !dimsOK) {			/* expecting header? */
		char	*err = cm_getheader_ncomp(&dtype, &nrows, &ncols,
					&swap, &ncomp, scale, fp);
		if (err)
			error(USER, err);
	...
	case DTxyze:
		if (ncomp != 3)
			error(USER, "unexpected # components for picture matrix");
	...
	if (nrows <= 0) {			/* don't know length? */
		...
		if (cm_elem_size[dtype] && (fp != stdin) & (inspec[0] != '!')) {
			...
			if (fseek(fp, 0L, SEEK_END) == 0) {
				long	rowsiz = (long)ncols*cm_elem_size[dtype]*ncomp/3;
	...
	if (dtype == DTascii) {				/* read text file */
		...
		for (r = 0; r < maxrow; r++) {
			...
			if (ncomp == 1) {
				if (fscanf(fp, sizeof(COLORV)==sizeof(float) ?
						"%f" : "%lf", cv) != 1) {
					if ((nrows <= 0) & (r > 0) & !c) {
				...
				cv[1] = cv[2] = cv[0];
			} else if (fscanf(fp, COLSPEC, cv, cv+1, cv+2) != 3) {
				if ((nrows <= 0) & (r > 0) & !c) {
					cm = cm_resize(cm, maxrow=r);
					break;
				} else
		...
		if ((ncomp == 3) & (sizeof(COLOR) == cm_elem_size[dtype])) {
			size_t	nread = 0;
			do {				/* read all we can */
			...
			if (n <= 0)
				goto not_handled;
			while (n--) {
				if (getbinary(dc, sizeof(double), ncomp, fp) != ncomp)
					goto EOFerror;
				if (swap) swap64((char *)dc, ncomp);
				if (ncomp == 1)
					setcolor(cvp, dc[0], dc[0], dc[0]);
				else
					copycolor(cvp, dc);
				cvp += 3;
			}
			...
			if (n <= 0)
				goto not_handled;
			while (n--) {
				if (getbinary(fc, sizeof(float), ncomp, fp) != ncomp)
					goto EOFerror;
				if (swap) swap32((char *)fc, ncomp);
				if (ncomp == 1)
					setcolor(cvp, fc[0], fc[0], fc[0]);
				else
					copycolor(cvp, fc);
				cvp += 3;
			}
